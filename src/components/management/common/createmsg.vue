...
/*import wangEditor from "../../utils/wangEditor.js"*/
	export default {
	    ...
	    methods:{
	    	list(){//面包屑跳转路由
		          let that = this;
		          that.$router.push({ path: 'messagetable'})
		    },
			submit(){//发表
				let that = this;
			    ...
			    let mark = this.$route.query.id;
			   	if(title != "" && content != ""){
				    ...
				    this.$Modal.confirm({
	                    ...
	                    content: "是否更新文章？",
	                    onOk () {
	                      this.$http.post("/api/ManagementUpdate",{"title":title,"autherId":autherId,"content":content,"id":mark}).then((response)=>{				   
						   		 that.$router.push({ path: 'messagetable' })	
						})
	                    }
	                })			      
	     		...
	     		//发表
		     		this.$Modal.confirm({
	                    ...
	                    content: "是否发表文章？",
	                    onOk () {
	                      this.$http.post("/api/ManagementAdd",{"title":title,"autherId":autherId,"content":content}).then((response)=>{							   
							    that.$router.push({ path: 'messagetable' })	
						})
	                    }
	                })
			...
			replay(mark){//返显
				this.$http.post("/api/ManagementReplay",{"id":mark}).then((response)=>{
